...
**리덕스 사가 - Axios 사용해 서버와 통신하기**

리덕스 사가와 Axios 로 서버와 비동기 통신하는 부분에 대한 글을 작성했다. Axios 는 새 객체를 만들어 사용하는 방식으로 이후 수정해야 할 사항이 발생했을 때 조금 더 쉽게 수정할 수 있도록 만들었고, 통신시 요청 전송, 성공, 실패 세 부분으로 나눠서, 성공했을 때와 실패했을 때 액션을 디스패치하는 제너레이터 함수를 리턴하는 사가 유틸 함수를 생성한 과정을 적었다.
...
인사이드 자바스크립트 4장을 읽으면서 내가 모르던 함수의 특징들을 많이 알게 되었다. 일단 함수도 객체이기 때문에 프로퍼티들을 동적으로 생성할 수 있다는 점과 리터럴 함수, 생성자 함수 별 함수 초기화 과정. 그리고 생성자 함수를 사용할 때 유용한 패턴 등에 대해서 알게 되었다.

약간 어려운 내용들이 주를 이루고 있어서 다시 한번 읽으면서 내용을 블로그에 올려야겠다고 생각이 들었다. 다음 한 주 간 목표는 이 내용을 습득하고, 정리해서 블로그에 올리는 것이다.
...
어제 읽은 내용을 조금 정리해보았다. 아직 완벽히 정리하지 않아서 블로그에는 올리지 않았는데 한 주간 전부 정리해서 올릴 예정이다.
...
* 홈 화면 디자인 및 로그아웃 추가
* 로그인 실패 후 회원가입 이동시 실패 메시지가 남아있던 이슈
  * 로그인 에러와 회원가입 에러를 분리하여 처리
...
금방 끝날 줄 알았는데 삽질을 좀 했다. 중요한 것은 conf 파일에 root 값을 넣을 때, 지금은 /var/share/nginx 내에 있는 디렉터리를 사용해야 제대로 띄워진다는 점이다. 파일을 만든 권한때문인지 아직 파악하지 못했는데, 여기다가 넣지 않을 경우 500 에러가 발생한다.

---

## 2021.08.05

**친구 위키 네이버 클라우드에서 서비스**

네이버 클라우드 1년 무료 마이크로 서버에서 React - Nginx - Nodejs - MongoDB 를 사용해서 서비스를 시작했다. 아직 회원가입과 로그인 기능 뿐인데, 네이버 클라우드 공인 IP 를 3개월 동안 밖에 이용할 수 없기 때문에 일단 서비스를 시작했다. 글 쓰기 기능 및 다른 기능들을 하나씩 추가하면서 1 버전을 빨리 배포하는 것이 목표다. 3 개월 이내에 1 버전 정식 릴리즈를 목표로 달려보겠다!
...
**<a href="./javascript/js 함수에 대하여.md">JS 함수 정리</a> 완료**

그동안 자바스크립트의 함수에 대해 모르고 사용해왔다는 것을 느끼는 시간이었다. 그래도 이번에 정리를 통해서 어느 정도 자바스크립트에서의 함수 동작 방식과 this 바인딩에 대해 이해할 수 있게 되었다.
...
포스트 기능 서버에서만 구현.(아직 DB 연동은 하지 않음)
...
캡슐화를 통해 제품에 대한 이해를 높힐 수 있고, 코드 수정을 더 쉽게 해준다.

---

## 2021.08.13

**객체 지향 프로그래밍 입문 - 캡슐화 예제**

데이터를 받아오기만 하는 부분은 캡슐화 하는 것이 좋고, 추가적인 처리까지도 캡슐화 하는 것이 좋음.
...
비동기 문제가 코딩테스트로 나온적은 처음이었다. 너무 신박했고 새로운 경험이었다. 재밌었고, 역시 토스라는 점이 코딩테스트 문제에서도 느껴졌다.



**객체 지향 프로그래밍 입문 - 다형성과 추상화**

추상화의 경우 이전의 캡슐화와 다르게 너무 자주하면 오히려 안좋아 질 수 있다. 추상화를 하게 되면 결국 타입이 하나 추가되는 것이고 이 것이 복잡성 증가로 이어질 수 있기 때문에 꼭 추상화가 필요한 경우(추상화를 통해 더 나은 코드를 만들 수 있는 경우)에만 추상화를 해야 한다.
...
메뉴 컴포넌트를 생성했는데, 메뉴에는 유저 리스트들과 유저를 클릭하면 유저에 속한 글들의 리스트가 보이도록 구현했다. 메뉴 컴포넌트 내에 메뉴 아이템 컴포넌트가 존재하는 형태로 구성했다.

메뉴 컴포넌트는 메뉴의 위치를 설정하고, 메뉴가 보이고 보이지 않는 동작들을 하는 컴포넌트이다.

메뉴 아이템 컴포넌트는 1 명의 유저와, 유저에 속한 글들을 가지고 있으며, 유저를 클릭하면 글들이 리스트형태로 나타나는 컴포넌트이다.

---

## 2021.08.25

**친구위키 - 배포버전 네이버 클라우드에서 AWS 로 이전**

네이버 클라우드 기존 받은 100,000 원 크레딧이 거의 다 사용돼서, 어쩔수 없이 옮겼다. 네이버 클라우드의 경우 마이크로서버는 1년 무료이지만, 외부에서 ssh 접속 말고, tcp 로 들어갈 때는 별도로 공인 IP 를 발급 받아야 하는데, 이게 하루에 2,700 원이라서 사실상 배포하려면 3개월도 사용하지 못한다. 1년간 마이크로서버가 무료인게 무슨 의미인가 싶은데 왜 이런 정책을 펴는지 궁금하다.

어쨌든 기존 네이버 클라우드의 내용을 AWS 로 이전하면서 nginx 설정도 다시하면서 이전 내용을 상기시키는 좋은 기회가 됐다.

```
server {
	root /var/share/nginx/html; 			# 빌드된 파일의 경로
	index index.html index.htm;
	try_files $uri $uri/ /index.html;		# CSR 을 위해 설정
	location /api/ {
		proxy_pass http://localhost:4000;	# 서버와의 통신점
	}
}
```
...
**친구위키 - 다른 사람의 글 수정 기능 추가**

수정 버튼을 현재 로그인한 유저 상태와 글의 유저가 같을 경우 나타나지 않도록 하였고, 다를 경우 수정 버튼이 나타나도록 하여 글 수정 기능 추가함.

---

## 2021.08.27

**친구위키 - 글 수정시 수정한 사람 및 수정 시간 보이도록 수정**

서버에서 publishedDate 값이 초기 생성할 때 디폴트(현재)로 생성되고, 이후 수정할 때 publishedDate 를 변경하지 않아서 글 수정 요청이 들어왔을 때 publsihedDate 값이 현재 시간으로 수정되도록 변경함.
...
기존 수정 버튼 옆에 삭제 버튼을 추가해 자신의 글이 아닐 경우 삭제 기능 추가함.

자신의 글일 경우 아예 삭제 버튼이 나타나지 않고, 다른 사람의 글일 경우 삭제 버튼 클릭하면 모달이 등장하고, 모달에서 확인 버튼을 클릭하면 최종 삭제되도록 구현.
...
**친구위키 - 로그인 후 첫 포스트 작성시 작성되지 않는 이슈 해결**

초기화 액션 디스패치의 문제였다. 기존에 `owner` 를 매개변수로 받으면 INITIALIZE 타입의 액션을 생성하는 initialize 액션 생성 함수를 사용했었는데, 이름과 다르게 마지막에 컴포넌트가 마운트 해제될 때 initialize 를 사용했고, 따라서 처음 write 페이지에 접속했을 때 owner 를 설정하는 코드가 없었다. 따라서 initialize 의 역할을 owner 를 설정하는 것으로 바꿨고, clear 액션 생성 함수를 만들어서, CLEAR 타입의 액션이 디스패치되면 write 초기 상태를 빈 값으로 초기화하도록 하여, initilize 는 컴포넌트가 마운트 되었을 때, clear 는 언마운트 될 때 디스패치 하도록 수정했다.
...
만약 "손흥민" 을 검색할 때 손 까지 입력했을 때 하단에 [손, 손금, 손흥민] 과 같은 리스트들이 뜨고 솒 까지 입력했을 때 하단의 목록이 지워지지 않고 남아있게 프론트에서 어떻게 구현할 수 있을지에 대해 오늘 질문을 받았고, 몰라서 대답하지 못했는데 두 키워드를 알려주셔서 찾아보게 되었다. 

디바운싱은 특정 기간 사이에 여러 이벤트가 발생했을 때 마지막 이벤트만 발생시키는 것이고, 스로틀은 특정 기간에 최대 한 번만 이벤트가 발생하게 하는 것이다.

다르게 말하면 디바운싱은 같은 동작을 연속적으로 30번 하면 마지막 동작만 이벤트 발생시키게 되고, 스로틀은 만약 기간 설정을 1초로 해 놓으면 같은 동작을 연속적으로 30번 하면 1 초마다 최대 하나가 추가되므로 1초에 한 번씩 동작을 수행하면 30번 이벤트가 발생할 수 있고, 1초에 15번 동작하면 2 번 이벤트가 발생하게 되는 것이다.
